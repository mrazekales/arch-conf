...
#
# project:    https://github.com/mrazekales/myarch-desktop
#
# original project: https://github.com/MatMoul/archfi
...
apptitle="Arch Linux Base Install - Version: 0.1 (LGPLv3)"
skipfont="0"
...
diskpartmenu(){
  ...
  options+=("$txtautoparts (gpt,noswap)" "swapfile")
  options+=("$txtautoparts (dos,noswap)" "swapfile")
  if [ "$eficomputer" != "0" ]; then
    options+=("$txtautoparts (gpt,efi)" "")
    options+=("$txtautoparts (gpt,efi,noswap)" "swapfile")
    options+=("$txtautoparts (gpt,bios+efi,noswap)" "swapfile")
  fi
...
installmenu(){
  ...
  options+=("$txtinstallarchlinux" "base system (pacstrap)")
...
# base, base-devel
installdevel(){
    if (whiptail --backtitle "$apptitle" --title "$txtbasedev" --yesno "$txtefibootmgr" --defaultno 0 0) then
        $installtype="base base-devel"
      fi
}
...
installbase(){
  installdevel
  clear
  echo "pacstrap /mnt $installtype"
  pacstrap /mnt $installtype
  pressanykey
}
...
loadstrings(){
  ...
  txtbasedev="Devel package?"
  txtinstalldev="Do you want to instal 'base-devel' packages?"
  txtinstall="Install Arch"
